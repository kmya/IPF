...
# Fit a linear SVM model and obtain feature weights
    ...
    svmModel = svm(train.data[, -1], train.data[, 1], cost=100, cachesize=500,gamma = 1,
    ...
    svm.list = lapply(results, function(x) {
            tObj <- tune(svm,
                      ...
                      ranges       = tune(svm,
                                          ...
                                          train.y = input[x$train.data.ids, 1],kernel="linear",

                                          ranges  = list(gamma=c(0.1,1,100), cost=c(1,10,100,1000)))$best.par,

                      # tunecontrol  = tune.control(sampling='fix')
                      )
            BestSvm<-tObj$best.model
            yPred<-predict(BestSvm,input[x$train.data.ids, 1+x$feature.ids[1:i]])
            (ConfM<-table(yPred,input[x$train.data.ids, 1]))
            #tObj$best.model$index
